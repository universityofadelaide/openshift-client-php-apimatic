...
/**
 * A BuildPostCommitSpec holds a build post commit hook specification. The hook executes a command in a
 * temporary container running the build output image, immediately after the last layer of the image is
 * committed and before the image is pushed to a registry. The command is executed with the current
 * working directory ($PWD) set to the image's WORKDIR.
 *
 * The build will be marked as failed if the hook execution fails. It will fail if the script or
 * command return a non-zero exit code, or if there is any other error related to starting the
 * temporary container.
 *
 * There are five different ways to configure the hook. As an example, all forms below are equivalent
 * and will execute `rake test --verbose`.
 *
 * 1. Shell script:
 *
 * "postCommit": {
 ...
 *
 * It is invalid to provide both Script and Command simultaneously. If none of the fields are specified,
 * the hook is not executed.
 */
class V1BuildPostCommitSpec implements JsonSerializable
{
    /**
     ...
     * @var string|null $script public property
     */
    public $script;
     ...
     * @param string $script  Initialization value for $this->script
     */
    public function __construct()
    {
        if (3 == func_num_args()) {
            $this->args    = func_get_arg(0);
            $this->command = func_get_arg(1);
            $this->script  = func_get_arg(2);
        }
    }
